    ...
    <v-row no-gutters class="fill-height">
      ...
      <v-col cols="12" md="5" lg="4" class="loan-form-side">
        <div class="loan-form-container">
          ...
          <div class="form-progress">
            ...
            <v-progress-linear :model-value="overallProgress" color="primary" height="6" rounded />
          </div>
          ...
          <div class="form-sections">
            <v-expansion-panels v-model="expandedPanels" multiple variant="accordion">
              <v-expansion-panel v-for="(section, sectionIndex) in FORM_SECTIONS" :key="section.title"
                :value="sectionIndex" class="form-expansion-panel" :class="{
                ...
                <v-expansion-panel-title>
                  ...
                  <div class="section-status">
                    ...
                    <v-icon v-if="sectionCompletionStatus[sectionIndex]?.percentage === 100" color="success"
                      class="status-icon">
                      mdi-check-circle
                    </v-icon>
                    <v-icon v-else-if="sectionCompletionStatus[sectionIndex]?.hasErrors" color="error"
                      class="status-icon">
                      mdi-alert-circle
                    </v-icon>
                    <v-icon v-else color="grey" class="status-icon">
                      mdi-circle-outline
                    </v-icon>
                  </div>
                </v-expansion-panel-title>
                ...
                <v-expansion-panel-text>
                  <div v-for="field in section.fields" :key="field.variable" class="form-field"
                    :class="{ 'number-field': field.type === 'number' }">
                    <!-- テキストフィールド -->
                    <v-text-field v-if="field.type === 'text-input'" v-model="formData[field.variable]"
                      :label="field.label" :maxlength="field.maxLength" :error-messages="errors[field.variable]"
                      variant="outlined" color="primary" @blur="validateSingleField(field.variable)"
                      @input="clearFieldError(field.variable)" />

                    <!-- 数値フィールド -->
                    <v-text-field v-else-if="field.type === 'number'" v-model.number="formData[field.variable]"
                      :label="field.label" :error-messages="errors[field.variable]" type="number" variant="outlined"
                      color="primary" @blur="validateSingleField(field.variable)"
                      @input="clearFieldError(field.variable)" />

                    <!-- テキストエリア -->
                    <v-textarea v-else-if="field.type === 'paragraph'" v-model="formData[field.variable]"
                      :label="field.label" :maxlength="field.maxLength" :error-messages="errors[field.variable]"
                      variant="outlined" color="primary" rows="3" auto-grow counter
                      @blur="validateSingleField(field.variable)" @input="clearFieldError(field.variable)" />

                    <!-- セレクト -->
                    <v-select v-else-if="field.type === 'select'" v-model="formData[field.variable]"
                      :label="field.label" :items="field.options" :error-messages="errors[field.variable]"
                      variant="outlined" color="primary" @blur="validateSingleField(field.variable)"
                      @update:model-value="clearFieldError(field.variable)" />

                    <!-- ファイル選択 -->
                    <div v-else-if="field.type === 'file-list'">
                      <DifyFileUpload v-model="uploadedFileIds" :title="field.label" :multiple="true"
                        :accepted-types="['.pdf', '.doc', '.docx', '.xls', '.xlsx', '.txt', '.csv']" :max-size-mb="50"
                        placeholder="決算書（3期分）を選択してください" @upload-complete="onFileUploadComplete"
                        @upload-error="onFileUploadError" />

                      <!-- エラーメッセージ -->
                      <v-messages v-if="errors[field.variable]?.length" :messages="errors[field.variable]" color="error"
          ...
          <div class="generate-button-container">
            <div class="button-group">
              <v-btn class="generate-button" :loading="isGenerating" :disabled="isGenerating" @click="generateReport">
                <v-icon left>mdi-file-document-outline</v-icon>
                稟議書生成
              </v-btn>
              <v-btn class="clear-button" :disabled="isGenerating" @click="clearForm">
      ...
      <v-col cols="12" md="7" lg="8" class="report-output-side">
        <div class="report-container">
          ...
          <!-- ワークフロー進捗表示 -->
          <DifyWorkflowProgress v-if="workflowExecutionState.status !== 'idle'"
            :execution-state="workflowExecutionState" :show-actions="true" class="mb-4" @retry="retryWorkflow"
            @cancel="cancelWorkflow" @reset="resetWorkflow" />

          <!-- ストリーミングMarkdown表示コンポーネント -->
          <DifyStreamingMarkdown :content="streamingReportText" :is-streaming="isStreaming" :show-actions="true"
            :auto-scroll="true" class="report-markdown-viewer" />
        </div>
      </v-col>
    </v-row>
  </v-container>
